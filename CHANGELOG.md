...
exists and points at the challenge the video showed: `/draw-an-owl-from-memory`,
carrying `utm_source=youtube`, `utm_medium=shorts`, `utm_campaign=cydi_shorts`
and the Short's own video id as `utm_content`.

The id is `FuEiFnzQuSY`, taken from the published video's own link rather than
read off a screen - it alternates case in a way nobody would reproduce by eye,
and one wrong character would file every click under a video that does not exist.
`worker/campaignLinks.test.ts` pins the redirect, the landing path and the
attribution the tags resolve to, as it does for the cat, the dog and the bear.

`/s/cat`, `/s/dog` and `/s/bear` are untouched, `/s/` stays out of the sitemap and
...
nobody has unlocked yet without giving anything away.

**Two new shape pages.** `/draw-a-perfect-star` and `/draw-a-perfect-heart` join
the existing three, each with its own title, H1, meta description and copy - the
star's about arm length, tip spacing and ten straight edges, the heart's about the
mirror axis, matching lobes and a bottom point on the centre line. Both carry a
crawlable illustration under `public/images/seo/`, generated by
`npm run seo-images` from the same generator functions the game draws the target
from (`scripts/generateSeoShapeImages.ts`), so a picture can never drift from the
shape it promises. Canonical URLs and sitemap entries come free from the existing
`worker/seoPages.ts` machinery.

**`/draw-shapes-online` is now the hub.** A visible "Practice individual shapes"
section links to Circle, Star and Heart with plain `<a href>` links and a line
each on what makes that shape hard; the shape pages link back to it and to each
other. Its existing copy is untouched. Its CTA moved to the accuracy test, since
the circle now appears in the practice list and a page shouldn't offer the same
destination twice.

**Practice rounds.** The star sits thirteen shapes into Geometric and the heart is
the first shape of a category that costs 1000 coins, so a plain deep link would
have dropped every new visitor on the category map instead of the shape the page
promised. Those two pages now open a practice round: the real canvas, the real
scoring, the real overlay - and nothing kept. No best score, coins, achievements,
completion, category unlock or round counters, which also means an SEO visitor's
99 can never quietly consume the reward a player should still earn later by
legitimate play. `src/app/shapeRoundOutcome.ts` is now the single place a finished
round writes anything, and a practice round resolves to "nothing to write".
Leaving one exits to the category list rather than a locked category's map.

**Practice rounds are visible in analytics, never mixed in.** They report under
their own game type (`seoPractice`) and their own completion event
(`shape_practice_completed`), so SEO-practice usage is measurable while the
shapeChallenge funnel and the report's average score and pass rate stay a
real-play baseline.
