 ...
 */
// THE single source of the Privacy Policy text.
//
// Two very different renderers read this one string:
//   - src/screens/PrivacyPage.tsx  - the React page, which is what the Android
//     WebView and `npm run dev` show at /privacy.
//   - worker/contentPages.ts       - the server-rendered public page, which is
//     what a browser (and a crawler) gets from playcydi.com/privacy.
//
// Before this module existed the policy lived only in the React component, so
// the public /privacy URL served a 2.4 KB empty SPA shell with no policy text in
// it at all - invisible to anything that does not run JavaScript.
//
// RULE: this file must stay dependency-free (no imports, no import.meta, no
// Vite-injected globals) for the same reason engine/scoringConstants.ts does -
// the Worker bundles it, and anything unbundleable there throws at module load.
//
// The markup is plain HTML rather than JSX so both renderers can use it
// verbatim: React injects it with dangerouslySetInnerHTML (safe - this is
// static, self-authored content with no user input anywhere in it), and the
// Worker concatenates it into the page document.
 ...
 * line, so each renderer can wrap it in its own chrome (the app's card, or the
 * public page's <main>).
 */
